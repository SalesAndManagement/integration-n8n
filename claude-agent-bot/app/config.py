...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        usernames = frozenset(name.lstrip("@").lower() for name in _csv("TELEGRAM_ALLOWED_USERNAMES"))

        if not user_ids and not usernames:
            raise ConfigError(
                "Не задано ні TELEGRAM_ALLOWED_USER_IDS, ні TELEGRAM_ALLOWED_USERNAMES. "
                "У агента є доступ до файлів і мережі, тому бот не стартує без явного "
                "списку дозволених користувачів."
            )
