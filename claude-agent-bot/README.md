...
Перше, що варто запустити, коли щось не працює:

```bash
./scripts/diagnose.sh
```

Він іде ланцюгом і зупиняється на тому місці, де ламається: читає `.env` → б'є напряму
в Anthropic API (це відсікає питання ключа, workspace і кредитів, і показує заголовок
`anthropic-workspace-id` з відповіді) → запускає Claude Code через SDK тими самими
опціями, що й бот → просить агента відкрити сторінку браузером.
