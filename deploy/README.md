...
### Варіант Б — безкоштовно, без покупки домену

* **Хостнейм від Contabo.** Кожному VPS видається робоче ім'я виду
  `vmi1234567.contaboserver.net`, яке вже резолвиться на ваш IP. Його можна
  одразу вписати в `N8N_DOMAIN` — сертифікат випуститься. Мінус: некрасиво
  і прив'язано до конкретного сервера.
* **DuckDNS / аналоги.** Реєструєте `щось.duckdns.org` і вказуєте свій IP.
  Підходить для тестів; для клієнтських інтеграцій краще свій домен.
